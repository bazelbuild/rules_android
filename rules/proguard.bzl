...
def _get_proguard_specs(
    ...
    if ctx.configuration.coverage_enabled and hasattr(ctx.attr, "_jacoco_runtime"):
        proguard_deps.append(ctx.attr._jacoco_runtime[ProguardSpecInfo])
...
def _optimization_action(
    ...
    Args:
      ...
      keep_radius: File. Optional file used to write the "keep radius" proto, which is a mapping of
        keep rules to their kept classes and members.
    ...
    if keep_radius:
        args.add("-printseeds", keep_radius)
        outputs.append(keep_radius)
...
def _create_empty_proguard_output(
    ...
    Args:
      ...
      keep_radius: File. The output keep radius proto.
...
def _create_optimization_actions(
    ...
    if not num_passes:
        _optimization_action(
            ...
            keep_radius = outputs.keep_radius,
