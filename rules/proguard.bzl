...
def _get_proguard_output(
    ...
    # TODO(timpeut): this is a google3 internal artifact
    proto_mapping = None
    if getattr(ctx.attr, "proguard_generate_mapping", False):
        proto_mapping = _get_proguard_temp_artifact(ctx, "_proguard.pbmap")
...
def _create_optimization_actions(
    ...
    if not num_passes:
        _optimization_action(
            ...
            proguard_output_proto_map = outputs.proto_mapping,
