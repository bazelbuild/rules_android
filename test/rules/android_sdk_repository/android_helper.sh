...
function set_up_rules() {
  local repo_rule="$(rlocation build_bazel_rules_android/rules/android_sdk_repository.bzl)"
  local repo_rule_dir="$(dirname "${repo_rule}")"
  local dest_dir="${TEST_TMPDIR}/android_sdk_repository_src"
  mkdir -p "${dest_dir}/rules"
  cp -r "${repo_rule_dir}"/* "${dest_dir}/rules"
...
load("@android_sdk_repository_src//rules:android_sdk_repository.bzl", "android_sdk_repository")
EOF
}
...
def _show_sdk_info_impl(ctx):
    provider = ctx.attr.android_sdk[AndroidSdkInfo]
...
EOF
  ...
  "${BIT_BAZEL_BINARY}" build --experimental_google_legacy_api //sdk_check:check >& $TEST_log || fail "Expected success"
